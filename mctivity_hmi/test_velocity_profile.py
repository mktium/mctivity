...
class VelocityProfileHmiTests(unittest.TestCase):
    ...
    def test_velocity_start_precedes_position_fallback(self):
        ...
        self.assertIn("const direction = liveTargetCps * AXIS_DIR < 0 ? -1 : 1;", source)
        self.assertIn("velocityCommandCountsS(direction * requestedRpm, device)", source)
        self.assertIn("function velocityCommandCountsS(value, device = activeDevice)", source)
        self.assertIn("return rpmToCountsS(value, device) * AXIS_DIR;", source)
        self.assertIn("}, 150);", source)
        self.assertIn("if (syncVelocityEnabled) return syncJogVelocity(v);", source)
        self.assertIn("return api({cmd:'jog_velocity', velocity:velocityCommandCountsS(v, activeDevice)});", source)
