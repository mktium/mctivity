...
static int ready_to_switch_on(uint16_t sw)
{
    return (sw & 0x006f) == 0x0021;
}

static int switched_on(uint16_t sw)
{
...
static uint16_t next_controlword(uint16_t sw)
{
    ...
    if (ready_to_switch_on(sw)) {
        return 0x0007;
    }
    if (switched_on(sw) || operation_enabled(sw)) {
        return 0x000f;
    }
    return 0x0006;
}

static int32_t ramp_target(int32_t start, int32_t delta, uint32_t step, uint32_t total_steps)
{
    if (step >= total_steps) {
        return start + delta;
    }

    // Smoothstep avoids a hard velocity jump at the beginning and end.
    double x = (double)step / (double)total_steps;
    double s = x * x * (3.0 - 2.0 * x);
    return start + (int32_t)(delta * s);
}
...
int main(int argc, char **argv)
{
    ...
    int have_start_position = 0;
    int reached_enabled = 0;
    int failed = 0;

    if (argc > 1) {
        delta_counts = (int32_t)strtol(argv[1], NULL, 0);
    }
    if (argc > 2) {
        move_ms = atoi(argv[2]);
    }
    ...
    if (delta_counts == 0 || move_ms <= 0 || hold_ms < 0) {
        fprintf(stderr, "usage: %s [delta_counts=1000] [move_ms=2000] [hold_ms=2000]\n", argv[0]);
        return 2;
    }

    move_cycles = (uint32_t)move_ms;
    hold_cycles = (uint32_t)hold_ms;
    ...
    if (!sc) {
        fprintf(stderr, "failed to get MCTIVITY slave config\n");
        ecrt_release_master(master);
        return 1;
    }
    ...
    printf("MCTIVITY CSP one-way step: delta=%d counts, move=%d ms, hold=%d ms.\n",
           delta_counts, move_ms, hold_ms);
    ...
    while (running && cycles < 30000U) {
        ...
        if (!have_start_position && domain_state.working_counter >= 3 &&
            domain_state.wc_state == EC_WC_COMPLETE) {
            start_position = pos;
            target_position = start_position;
            have_start_position = 1;
            printf("Latched start position: %d; final target: %d\n",
                   start_position, start_position + delta_counts);
            fflush(stdout);
        }
        ...
        uint16_t cw = have_start_position ? next_controlword(sw) : 0x0000;

        if (have_start_position && operation_enabled(sw)) {
            reached_enabled = 1;
            cw = 0x000f;

            if (enabled_cycles < move_cycles) {
                target_position = ramp_target(start_position, delta_counts, enabled_cycles, move_cycles);
            } else {
                target_position = start_position + delta_counts;
            }
            enabled_cycles++;
        }
        ...
        EC_WRITE_S32(domain_pd + off_target_position, target_position);
        ...
        if (reached_enabled && enabled_cycles >= move_cycles + hold_cycles) {
            break;
        }
    ...
    printf("Disabling drive output at final target=%d...\n", target_position);
    ...
    if (failed || !reached_enabled) {
        fprintf(stderr, "one-way step failed or did not reach operation enabled\n");
        return 1;
    }

    printf("One-way CSP step completed and drive output disabled.\n");
    return 0;
}
