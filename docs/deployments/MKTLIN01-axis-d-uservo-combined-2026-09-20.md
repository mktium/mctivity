...
现场确认速度模式的“反转”按钮实际向右运动，而机械约定要求向左运动。原因是位置模式
已经使用 `AXIS_DIR=-1` 做机械方向换算，原生 PV 速度命令却直接使用了未换算的正负号。
HMI 现将 PV 速度命令统一经过同一方向映射：速度模式“反转”对应机械左行，“正转”对应
机械右行；位置目标和行程端点坐标保持不变。该修复只改变 HMI 指令符号，不改 PDO、驱动器
参数或 motiond，部署时不发送运动命令。
